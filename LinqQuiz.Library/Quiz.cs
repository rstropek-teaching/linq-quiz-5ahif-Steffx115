...
namespace LinqQuiz.Library
{
    public static class Quiz
    {
        ...
        {
            return (from num in Enumerable.Range(1, (exclusiveUpperLimit > 1) ? exclusiveUpperLimit-1:1) orderby num descending where ((checked(num * num) % 7) == 0) select num * num).ToArray();
        }
        ...
        {
            return text.Trim().Distinct().Where(f => char.IsLetter(f)).Select((char let,int num)=> { return (let, text.Count(f => f == let)); }).ToArray();
        }
    }
}
